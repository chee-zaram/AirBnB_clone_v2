...
echo "<html><head><title>Test HTML File</title></head><body><h1>This is a test.</h1></body></html>" | sudo tee /data/web_static/releases/test/index.html
...
sudo sed -i '0,/^\(\s*\)root\s*.*$/s//\1root \/data\/web_static\/current\/;/' /etc/nginx/sites-available/default
